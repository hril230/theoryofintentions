...
def controllerToI(sparc_path, newGoal, maxPlanLen, new_executer, new_domain_info, subfolder):
	global executer
	...
	global my_sparc_path
	global my_domain_info
	...
	preASP_toi_file = subfolder+'pre_ASP_files/preASP_ToI.txt'
	preASP_refined_domain_file = subfolder + 'pre_ASP_files/preASP_refined_domain.txt'
	asp_toi_file = subfolder+'ASP_files/ASP_ToI.sp'
	asp_toi_diagnosing_file = subfolder + 'ASP_files/ASP_TOI_Diagnosis.sp'
	zoomed_domain_file = subfolder + 'ASP_files/zoomed_domain.sp'
	current_location = 'unknown'
	current_refined_location = 'unknown_cell'
	currently_holding = 'nothing'


	my_sparc_path = sparc_path
	numberActivities = 1
	numberSteps = 4
...
def getIndexesRelevantToGoal():
	return [my_domain_info.LocationBook1_index, my_domain_info.LocationBook2_index, my_domain_info.In_handBook1_index, my_domain_info.In_handBook2_index]
...
def runToIPlanning(input):
	...
	answerSet = subprocess.check_output('java -jar '+my_sparc_path + ' ' + asp_toi_file +' -A ',shell=True)
...
def diagnose():
	...
	answerSet = subprocess.check_output('java -jar '+my_sparc_path + ' ' + asp_toi_diagnosing_file +' -A ',shell=True)
...
def refine(action, history, current_step, version, current_refined_location, currently_holding):
    ...
    refined_answer = subprocess.check_output('java -jar '+my_sparc_path + ' zoomed_domain.sp -A ',shell=True)
