...
which holds a string that represents the head of the fluent and its first variable, for example 'loc(rob1', and
...
and if 'in_hand(rob1' is not part of the key set, then the robot is not holding anything in his hand.
Example {'loc(rob1,':'library', 'locked(library':None, 'loc(book1': 'library', 'loc(book2':'kitchen'}
...
class World(object):
	def __init__(self,new_domain_info_formatting,dic_initial_conditions, this_seed):
		...
		self.domain_locations = self.domain_info_formatting.get_all_constant_subsorts('#room')
		random.seed(this_seed)

	def __update_state(self,answer_split):
		self.dic_WorldState = {}
		my_fluents = [v[v.find('(')+1:v.rfind(',')] for v in answer_split if 'holds' in v]
	...
	def __runASPDomain(self,input):
		...
		asp = '\n'.join(asp_split)
		asp_file = 	self.domain_info_formatting.asp_World_file
		f1 = open(asp_file, 'w')
		f1.write(asp)
		f1.close()
		print (asp_file)
		answer = subprocess.check_output('java -jar '+ self.domain_info_formatting.sparc_path + ' ' +asp_file+' -A',shell=True)
		if answer == '' or answer == '\n':
	...
	def robotObserves(self,fluents_to_observe):
		robot_name = list(self.domain_info_formatting.get_all_constant_subsorts('#robot'))[0]
		observations_requests = ['try_observing('+robot_name+','+ fluent +',0).' for fluent in fluents_to_observe]
		input = self.domain_info_formatting.dic_state_to_holds_list(self.dic_WorldState,0) + observations_requests
		answer_split = self.__runASPDomain(input)
		holds_list = [v for v in answer_split if 'holds' in v]
		observed_list = [v for v in answer_split if 'directly_observed' in v]
